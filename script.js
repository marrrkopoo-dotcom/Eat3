...
let content = fs.readFileSync('app.jsx', 'utf-8');
...
content = content.replace(/\{[^}]+category:\s*"[^"]+"[^}]+\}/g, match => {
    if (!match.includes("navCategory")) {
        return match.replace('category:', 'navCategory: "Напої", category:');
    }
    return match;
});
...
if (end_idx !== -1) {
    ...
    let products_str = '\\n' + js_items.join('\\n') + '\\n';
    content = content.slice(0, end_idx) + products_str + content.slice(end_idx);
} else {
    // try fallback
    const fallback_idx = content.indexOf('];\\n');
    if (fallback_idx !== -1 && fallback_idx < 10000) {
        let js_items = [];
        food_items.forEach((item, i) => {
            let js = `    { \n        id: ${26 + i}, name: "${item.name}", price: ${item.price}, image: "images/ramune.png", navCategory: "${item.nav}", category: "${item.cat}",\n        details: { brand: "${item.brand}", country: "${item.country}", volume: "${item.vol}", calories: "${item.cal}", description: "${item.desc}" }\n    },`;
            js_items.push(js);
        ...
        let products_str = '\\n' + js_items.join('\\n') + '\\n';
        content = content.slice(0, fallback_idx) + products_str + content.slice(fallback_idx);
    }
}

fs.writeFileSync('app_new.jsx', content, 'utf-8');
console.log('Done');
