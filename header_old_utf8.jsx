        ...
        <header className="glass-header sticky top-0 z-40 transition-colors duration-300 border-b border-gray-100 dark:border-gray-800">
            <div className="container mx-auto px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-6">
                    <div className="flex items-center group cursor-pointer" onClick={() => navigateTo('shop', '╨Т╤Б╤Ц')}>
                        <img src="images/logo.svg?v=8" alt="╨╢╤Г╨╣╨║╨░" className="h-16 w-auto object-contain block select-none" />
                    </div>
                        ...
                        <input 
                            ...
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    ...
                                    if (activeView !== 'shop' || activeNav !== '╨Т╤Б╤Ц') {
                                        navigateTo('shop', '╨Т╤Б╤Ц');
                                    }
                                }
                            }}
                            placeholder="╨Я╨╛╤И╤Г╨║ ╤Б╨╛╨╗╨╛╨┤╨╛╤Й╤Ц╨▓ (╨╜╨░╨╖╨▓╨░, ╨║╨░╤В╨╡╨│╨╛╤А╤Ц╤П, ╤Б╨╝╨░╨║, ╨▒╤А╨╡╨╜╨┤)..." 
                    ...
                    {currentUser ? (
                        <div className="flex items-center gap-3">
                            <div className="hidden md:flex flex-col items-end">
                                ...
                                <span className="text-xs text-primary font-bold">╨Ю╤Б╨╛╨▒╨╕╤Б╤В╨╕╨╣ ╨║╨░╨▒╤Ц╨╜╨╡╤В</span>
                            </div>
                        ...
                        <button onClick={() => setIsAuthModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-dark dark:text-white rounded-xl font-bold transition-colors shadow-sm">
                            ...
                            <span className="hidden sm:inline">╨г╨▓╤Ц╨╣╤В╨╕</span>
                        </button>
                    )}
                    ...
                    <div className="fixed top-[72px] left-0 w-full bg-white dark:bg-darkBg z-40 border-b border-gray-200 dark:border-gray-800 shadow-2xl p-6 max-h-[80vh] overflow-y-auto animate-in slide-in-from-top-4 duration-300">
                        <div className="container mx-auto max-w-4xl">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold text-dark dark:text-white">╨Я╨╛╤И╤Г╨║ ╤В╨░ ╨д╤Ц╨╗╤М╤В╤А╨╕</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                <div>
                                    <h4 className="font-bold text-sm text-gray-500 mb-3 uppercase tracking-wider">╨Ъ╨░╤В╨╡╨│╨╛╤А╤Ц╤П</h4>
                                    <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-dark dark:text-white rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all font-bold">
                                        <option value="╨Т╤Б╤Ц">╨Т╤Б╤Ц ╤В╨╛╨▓╨░╤А╨╕</option>
                                        <option value="╨Э╨░╨┐╨╛╤Ч">╨Т╤Б╤Ц ╨╜╨░╨┐╨╛╤Ч</option>
                                        <option value="╨б╨╛╨╗╨╛╨┤╨╛╤Й╤Ц">╨б╨╛╨╗╨╛╨┤╨╛╤Й╤Ц</option>
                                        <option value="╨б╨╜╨╡╨║╨╕">╨б╨╜╨╡╨║╨╕</option>
                                        <option value="╨У╨░╨╖╨╛╨▓╨░╨╜╤Ц ╨╜╨░╨┐╨╛╤Ч">╨У╨░╨╖╨╛╨▓╨░╨╜╤Ц ╨╜╨░╨┐╨╛╤Ч</option>
                                        <option value="╨Р╨╖╤Ц╨░╤В╤Б╤М╨║╤Ц ╨╜╨░╨┐╨╛╤Ч">╨Р╨╖╤Ц╨░╤В╤Б╤М╨║╤Ц ╨╜╨░╨┐╨╛╤Ч</option>
                                        <option value="╨б╨╛╨║╨╕ ╨╖╤Ц ╤И╨╝╨░╤В╨╛╤З╨║╨░╨╝╨╕">╨б╨╛╨║╨╕ ╨╖╤Ц ╤И╨╝╨░╤В╨╛╤З╨║╨░╨╝╨╕</option>
                                        <option value="╨Х╨╜╨╡╤А╨│╨╡╤В╨╕╨║╨╕">╨Х╨╜╨╡╤А╨│╨╡╤В╨╕╨║╨╕</option>
                                    </select>
                                </div>
                                <div>
                                    <h4 className="font-bold text-sm text-gray-500 mb-3 uppercase tracking-wider">╨ж╤Ц╨╜╨░ (тВ┤)</h4>
                                    <div className="flex items-center gap-2">
                                        <input type="number" placeholder="╨Т╤Ц╨┤" value={priceRange.min} onChange={e => setPriceRange({...priceRange, min: e.target.value})} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-dark dark:text-white rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />
                                        <span className="text-gray-400">-</span>
                                        <input type="number" placeholder="╨Ф╨╛" value={priceRange.max} onChange={e => setPriceRange({...priceRange, max: e.target.value})} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-dark dark:text-white rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />
                                    </div>
                                </div>
                                <div>
                                    <h4 className="font-bold text-sm text-gray-500 mb-3 uppercase tracking-wider">╨Ъ╨░╨╗╨╛╤А╤Ц╤Ч (╨║╨║╨░╨╗)</h4>
                                    <div className="flex items-center gap-2">
                                        <input type="number" placeholder="╨Т╤Ц╨┤" value={calRange.min} onChange={e => setCalRange({...calRange, min: e.target.value})} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-dark dark:text-white rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />
                                        <span className="text-gray-400">-</span>
                                        <input type="number" placeholder="╨Ф╨╛" value={calRange.max} onChange={e => setCalRange({...calRange, max: e.target.value})} className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-dark dark:text-white rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="font-bold text-lg text-dark dark:text-white">╨а╨╡╨╖╤Г╨╗╤М╤В╨░╤В╨╕ ({filteredProducts.length})</h4>
                                    <span className="text-sm text-gray-500 hidden sm:inline">╨Э╨░╤В╨╕╤Б╨╜╤Ц╤В╤М Enter ╤Г ╨┐╨╛╤И╤Г╨║╤Г, ╤Й╨╛╨▒ ╨┐╨╡╤А╨╡╨│╨╗╤П╨╜╤Г╤В╨╕ ╨▓╤Б╤Ц</span>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                    {filteredProducts.slice(0, 5).map(product => (
                                        <div key={product.id} onClick={() => { setIsSearchOverlayOpen(false); navigateTo('product', activeNav || '╨Т╤Б╤Ц', product); }} className="bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-3 cursor-pointer hover:border-primary hover:shadow-md transition-all group flex flex-col items-center text-center">
                                            ...
                                            <div className="mt-auto font-extrabold text-primary text-sm">{product.price} тВ┤</div>
                                        </div>
                                    ))}
